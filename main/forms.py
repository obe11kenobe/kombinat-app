...
class TaskCreationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if user:
            other_leaders = User.objects.filter(led_department__isnull=False).exclude(pk=user.pk)
            own_department_employees = User.objects.filter(department=user.department)
            self.fields['assignee'].queryset = (other_leaders | own_department_employees).distinct()
